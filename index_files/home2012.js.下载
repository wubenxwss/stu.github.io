 ...
 * ==========================================
 * home.js
 * Copyright (c) 2011 d.stulip.org
 * Author: rpwt@stulip.org
 * ==========================================
 */
...
var CoolsiteAlign = (function(){
    ...
    if(AlignCookie.is("coolsite") && al==0){
        set("center");
    }else{
        set("left");
    }
...
/*20131224测试停止
    ...
    var codeUrl = 'http://ext.weather.com.cn/53888.js'
    var containerID = 'TOOL_115_COM_JS'
    ...
    else window.onload = window.loadGadget
	*/
/*天气加载类结束*/
...
var Calendar = (function(){
	...
	function showToday(){ 
		var weekStr = "日一二三四五六";
		var template = '<a href="http://tool.114la.com/?ct=live&ac=calendar" rel="nr" title="点击查看万年历">#{YY}年#{MM}月#{DD}日 星期#{week} #{cncal}</a>';
